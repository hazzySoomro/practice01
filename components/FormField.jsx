...
import hide from "../assets/icons/eye-hide.png";
    ...
    <View className={`space-y-2 ${otherStyles}`}>
      ...
      <View className="w-full h-16 px-4 bg-black-100 rounded-2xl   border-2 border-black-200 focus:border-secondary flex-row items-center">
        <TextInput
          ...
          secureTextEntry={title === "Password" && !showPasswort}
        />
        {title === "Password" && (
          <TouchableOpacity onPress={() => setShowPassword(!showPasswort)}>
            <Image source={!showPasswort ? eye : hide} className="w-6 h-6" />
          </TouchableOpacity>
        )}
      </View>
    </View>
